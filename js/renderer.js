...
class Renderer2D {
  ...
  _stubDirections(atomId) {
    ...
    // Compute free slot directions based on hybridization
    const free = [];
    if (hyb === 'sp2') {
      // sp2: trigonal planar, 120° between bonds
      if (nUsed === 0) {
        // No existing bonds → distribute evenly at 120°
        for (let i = 0; i < nFree; i++) {
          free.push((2 * Math.PI / total) * i);
        }
      } else if (nUsed === 1) {
        // One existing bond: place free slots at ±120° from it
        for (let i = 0; i < nFree; i++) {
          const sign = (i === 0) ? 1 : -1;
          free.push(used[0] + sign * 2.094); // ±120°
        }
      } else {
        // Two existing bonds: find largest gap for remaining slot(s)
        ...
        const baseAngle = gapStart + maxGap / (nFree + 1);
        for (let i = 0; i < nFree; i++) {
      ...
      if (nUsed === 0) {
        ...
        free.push(used[0] + Math.PI);
      }
    } else {
      // sp3: tetrahedral, use largest gap approach
      if (nUsed === 0) {
        const gap = 2 * Math.PI / total;
        for (let i = 0; i < nFree; i++) free.push(gap * i);
      } else if (nUsed === 1) {
        const opp = used[0] + Math.PI;
        const base = opp - (Math.PI * (nFree - 1)) / (nFree + 1) * 0.5;
        for (let i = 0; i < nFree; i++) {
          free.push(base + (2 * Math.PI * i) / (nFree || 1));
        }
      } else {
        let maxGap = 0, gapStart = 0;
        for (let i = 0; i < nUsed; i++) {
