...
class Molecule {
  ...
  getHybridization(atomId) {
    ...
    if (a.aromatic) return 'sp2';
    // Fallback: ring carbon with 2 carbon neighbors → sp2 (handles SMILES-parsed aromatic rings)
    if (a.el === 'C' && neigh.length === 2) {
      const nbrEls = neigh.map(nb => this.atoms.get(nb)?.el).filter(e => e === 'C');
      if (nbrEls.length === 2) return 'sp2'; // typical for aromatic ring carbons
    }
    return 'sp3';
  }
  ...
  normalizeStructure() {
    ...
    // Cache ring membership for getHybridization optimization
    const ringMemberSet = new Set();
    for (const cycle of allRings) {
      for (const id of cycle) ringMemberSet.add(id);
    }
    // Override getHybridization to use ring information efficiently
    this._getHyb = this.getHybridization;
    this.getHybridization = function(id) {
      ...
      if (a.el === 'C' && a.fgs.includes('CO')) return 'sp2';
      if (a.aromatic) return 'sp2';
      if (a.el === 'C' && neigh.length === 2 && ringMemberSet.has(id)) return 'sp2';
      return 'sp3';
    };
